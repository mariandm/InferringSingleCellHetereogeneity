...
- **burstmean_perrep.m :** Calculate average burst size per experimental replicate across timepoints

- **burst_bestfit.m :** Randomly sample a parameter space to find the best parameter combination that describes the effective burst size data given a model of latent period to burst size relationship

- **burst_bestfit_eclipse.m :** Randomly sample a parameter space to find the best parameter combination that describes the effective burst size data given a model of latent period to burst size relationship, accounting for eclipse period
...
- **burst_RSME_eclipse.m :** Calculate the root squared mean error (RSME) for single-cell experimental observations given a latent period to burst size model and a set of parameters, accounting for eclipse period
...
- **calculate_betaeff_eclipse.m :** Calculate expected beta effective given a latent period to burst size model and corresponding parameters, and a latent period distribution. See methods for mathematical description, accounting for eclipse period
...
- **LP_burstdistr_eclipse.m :** Calculate the latent period to burst size relationship for a model that integrates eclipse period variability

- **splitDistr_eclipse.m :** Calculate the mean and CV of the eclipse period and post-eclipse period distributions
...
- **LS_Burst_eclipse.m :**  Find the parameters that minimize the squared error for a latent period to burst size function that best describe the single-cell cumulative effective burst size data, assuming eclipse period. Required for SupplFig10. 
...
- **burstDistribution_predicted_eclipse.m :** Calculate the predicted burst size distribution based on the latent period distribution and latent period to burst size relationship, assuming eclipse period. Required for SupplFig10. 

- **visualCI_burstfit.m :** Calculate 95% confidence intervals for cumulative effective burst size data. Required for Fig3. 

- **visual_burstfit_eclipse.m :** Get the visual fit for the cumulative burst size assuming eclipse period. 

- **CI_burstdist.m :** Computes the mean and CV of the burst size distribution as predicted from the latent period to burst size relationship and latent period distribution.
...
- **RMSE_eclipse.m :**  Calculates the RMSE for a range of parameters assuming a linear model of production period to burst size relationship, accounting for latet period.
...
- **SupplFig4_ProportionInfected.m :** Supplementary Figure 4: Proportion of infected cells through sampling time points in single-cell lysis detection protocol. 

- **SupplFig5_CDFAccuracy.m :** Supplementary Figure 5: Latent period prediction accuracy when simulating the single-cell lysis detection protocol across a range of latent period distributions. 

- **SupplFig6_IncubationError.m :** Supplementary Figure 6: Effect of incubation times in observed latent period mean and CV. 

- **SupplFig7_ParameterEstimateComparison.m :** Supplementary Figure 7: Comparison of parameter estimated (Latent period mean, CV, burst size) using the population-level and single-cell data.
...
- **SupplFig10_BurstSizeFitModels_eclipse.m :** Supplementary Figure 10:  Cumulative effective burst size fit for different latent period to burst size relationship models, assuming eclipse period. 
 
- **SupplFig11_Sloppyv.m :** Supplementary Figure 11: RMSE values for a range of parameters in  the linear model of production time to burst size, assuming eclipse period.
