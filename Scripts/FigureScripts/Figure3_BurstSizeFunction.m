...
% Best estimate
plot(xs,thebest_betaeff,'Color','#64A4FD','LineWidth',1.5);
...
xlim([0,12.5]);
xticks(0:2:12);
legend('CI','Inference','Inferred lysis events','Direction','reverse');
legend('boxoff');
legend('Location','northwest');
...
plot(mh_minpoints_raw(2:end),mh_y_new_raw(2:end),'Color','#64A4FD','LineWidth',1.5);
plot(mh_minpoints(2:end),mh_y_new(2:end),'Color','k','LineWidth',1.5,'LineStyle','--');
% Means
scatter(nanmean(thisdata,[1,2]),27,'k','*','SizeData', 60);
scatter(61.5,27,'r','fill','diamond','SizeData', 40); % Population-level burst size inference
...
legend('Single-cell experiment',...
    ...
    'Average burst size',...
    'Population-level predicted burst size');
legend('boxoff');
%% Aesthetics
for i =1:3
    nexttile(i);
