function [best_fit] = burst_bestfit_eclipse(data,sample_time,CI_LP,var_p,model,nrand)
    % Randomly samples a parameter space to find the global 
    % minimum best parameter combination that describes the effective burst
    % size data accounting for an eclipse period
    % Modified from script ~/Scripts/Functions/burst_bestfit.m
    % Code by: Marian Dominguez-Mirazo, 2026

    % Set parameter search range  
    d_range = [0,7]; % Average of eclipse period (hr)
    ...
    for i = 1:nrand
        ...
        x0 = [randsamp(i,:)];
        % Search function
        [this_s, this_fval,exit_function] = fminsearch(@(pars)burst_RSME_eclipse(data,sample_time,CI_LP,var_p,pars,model),x0);
